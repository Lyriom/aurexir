# Casos de prueba — v0.6.0 · Checkout embebido + compra con un clic
...
| CP-0.6.0-04 | P1 | Formulario de envío | Campos nombre/tel/dirección/CP presentes; nombre precargado de la cuenta | ✅ |
| CP-0.6.0-05 | P1 | Sin clave Stripe | Aviso "pago no disponible" + botón pagar deshabilitado | ✅ |
...
| CP-0.6.0-13 | P1 | `intent` envía customer/locale/método/código | Body correcto en `POST /checkout/intent` | 🟡 staging |
| CP-0.6.0-14 | P1 | Admin sube imagen y crea perfume | `POST /admin/uploads` → url; producto visible en la tienda | 🟡 staging |

**Cómo correr los ✅:**
```bash
npm run test:e2e -- v0.6.0            # solo checkout embebido
npm run test:e2e                      # toda la suite
```

**Los 🟡 (Stripe real / backend):** en staging con `VITE_STRIPE_PUBLISHABLE_KEY`
de test (`pk_test_…`), backend con `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` de
test, y tarjeta `4242 4242 4242 4242` (fecha futura, CVC cualquiera). Verificar:
el pago confirma sin salir del sitio, la página de éxito aparece, llega el correo
